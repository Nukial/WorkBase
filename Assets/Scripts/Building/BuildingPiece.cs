namespace WorkBase.Building {
    ...
    public class BuildingPiece : MonoBehaviour {
        ...
        public Dictionary<string, SnapPoint> snapPointMap = new Dictionary<string, SnapPoint>();

        void Awake() {
            snapPoints = new List<SnapPoint>(GetComponentsInChildren<SnapPoint>());
            foreach(var sp in snapPoints) {
                RegisterSnapPoint(sp);
            }
        }
        ...
        public void RegisterSnapPoint(SnapPoint sp) {
            ...
            // Register in the dictionary by key
            string key = ExtractKeyFromName(sp.name);
            if (!string.IsNullOrEmpty(key) && !snapPointMap.ContainsKey(key)) {
                snapPointMap.Add(key, sp);
            }
        }

        private string ExtractKeyFromName(string name) {
